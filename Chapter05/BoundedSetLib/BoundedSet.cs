...
public class BoundedSet<T> where T : IEquatable<T>
{
    ...
    /// <summary>
    /// Add the given <paramref name="element"/>, If the addition brings the size beyond capacity, remove the oldest 
    /// <paramref name="element"/>. If the given <paramref name="element"/>/> is already present, make the 
    /// <paramref name="element"/> the newest one.
    /// </summary>
