...
public class Container
{
    ...
    {
        // If the two Container groups are already connected, don't do anything.
    ...
    /// <paramref name="amount"/> must not subtract more than container can hold.
    /// </exception>
    public void AddWater(double amount)
    {
        if (amount + (Amount * _group.Count) < 0)
        {
            throw new ArgumentOutOfRangeException(nameof(amount), "Must not subtract more than container can hold.");
        }
